...
fn calc_hemisphere_light(model: mat4x4<f32>, position: vec4<f32>, normal: vec3<f32>, ulight: UniformLight) -> HemisphereLight {
    ...
    let diffuse: vec4<f32> = max(dot(world_normal, light_normal), 0.0) * ulight.color * blightness;

    let r = calc_reflection_light(position, normal, light_normal, ulight.reflection);
